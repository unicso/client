...
<div>
  <items-slider2 :items="items" :name="'Предложения по сферам деятельности'"/>


  <section class="promotion_items" v-if="2==3">
    <div class="promotion__header">
      <h4>Предложения по сферам деятельности</h4>
      <div class="navigation">
        <div @click="toLeft(); stopTimer();" class="arrow_left" v-html="$store.state.icons.arrow"></div>
        <div @click="toRight(); stopTimer();" class="arrow_right" v-html="$store.state.icons.arrow"></div>
      </div>
    </div>

    <div  class="promotion__block buisness_items  content_block" :style="items_block_style" >
      <div class="item" v-for="(item, index) in items" :style="item_style" @click="$router.push(item.link)">
